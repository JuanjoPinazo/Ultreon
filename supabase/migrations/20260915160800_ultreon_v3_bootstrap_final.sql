-- Migration: ULTREON 3.0 Clinical Registry V3 Bootstrap (Final & Idempotent)
-- Description: Creates the definitive ultreon_registry_cases table and RPC for V3.
...
-- Clear previous policies on the table safely
DO $$ 
DECLARE 
    pol RECORD;
BEGIN 
...
CREATE POLICY hospital_user_insert_v3_cases ON public.ultreon_registry_cases
    FOR INSERT WITH CHECK (
        public.get_current_user_role() = 'hospital_user'
        AND hospital_id = public.get_current_user_hospital_id()
        -- Note: We do NOT enforce operator_id = auth.uid() here to allow coordinators
    );
...
BEGIN
    ...
    -- Verify operator belongs to this hospital
    SELECT EXISTS (
        SELECT 1 FROM public.hospital_operators
        WHERE hospital_id = p_hospital_id AND operator_id = p_operator_id
    ) INTO v_operator_valid;

    IF NOT v_operator_valid THEN
        RAISE EXCEPTION 'Invalid Operator: Operator is not associated with this hospital';
    END IF;
