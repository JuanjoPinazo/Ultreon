...
export default function TriadaUltreeon({ optimization }: TriadaUltreeonProps) {
  if (!optimization) {
    return (
      <div className="bg-card border border-border rounded-3xl p-6 md:p-8">
        <h2 className="text-base font-bold text-foreground mb-1">Triada ULTREON™</h2>
        <p className="text-xs text-muted-foreground mt-2">Sin resultados de optimización registrados</p>
      </div>
    );
  }
  ...
  const getBadgeColor = (value: string | undefined | null) => {
    ...
    if (value === 'na') return 'bg-muted text-muted-foreground border-border dark:border-slate-700';
    return 'bg-muted text-muted-foreground border-border dark:border-slate-700';
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-base font-bold text-foreground mb-1">Triada ULTREON™ Optimization</h2>
        <p className="text-xs text-muted-foreground">Métricas clave de expansión, aposición y optimización de bordes</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          ...
          {optimization.post_stent_msa_mm2 !== undefined && optimization.post_stent_msa_mm2 !== null && (
            <div className="mb-4">
              <p className="text-[10px] font-mono text-muted-foreground mb-1 uppercase">MSA Post-Stent</p>
              <p className="text-2xl font-bold text-cyan-400">{optimization.post_stent_msa_mm2.toFixed(2)}</p>
              <p className="text-[9px] text-muted-foreground font-mono">mm²</p>
            </div>
          )}
          ...
          {optimization.stent_expansion_percent !== undefined && optimization.stent_expansion_percent !== null && (
            <div className="mb-4">
              <p className="text-[10px] font-mono text-muted-foreground mb-1 uppercase">Expansión</p>
              <p className="text-2xl font-bold text-yellow-400">{optimization.stent_expansion_percent.toFixed(1)}%</p>
              <p className="text-[9px] text-muted-foreground font-mono">del diámetro referencia</p>
            </div>
          )}
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          ...
          {optimization.malapposition_length_mm !== undefined && optimization.malapposition_length_mm !== null && (
            <div className="mb-4">
              <p className="text-[10px] font-mono text-muted-foreground mb-1 uppercase">Longitud Malaposición</p>
          ...
          {optimization.requires_malapposition_correction && (
            <div className="mt-4 pt-4 border-t border-border bg-yellow-950/30 p-2 rounded-lg">
        ...
        <div className="bg-card border border-border rounded-2xl p-5">
          ...
          <div className="mb-3">
            <p className="text-[10px] font-mono text-muted-foreground mb-1 uppercase">Disección Proximal</p>
          ...
          <div className="mb-4">
            <p className="text-[10px] font-mono text-muted-foreground mb-1 uppercase">Disección Distal</p>
          ...
          {optimization.significant_flap_gt_3mm && (
            <div className="mt-4 pt-4 border-t border-border bg-red-950/30 p-2 rounded-lg">
          ...
          {optimization.requires_edge_treatment && (
            <div className="mt-2 pt-2 border-t border-border bg-yellow-950/30 p-2 rounded-lg">
      ...
      <div className="bg-background/60 border border-border rounded-2xl p-4">
        <p className="text-xs font-bold text-muted-foreground mb-3">ESTADO GENERAL TRIADA</p>
        <div className="grid grid-cols-3 gap-3 text-center">
          <div className={`p-2 rounded-lg text-[9px] font-mono ${optimization.adequate_expansion === 'yes' ? 'bg-emerald-950/60 text-emerald-400' : 'bg-card text-muted-foreground'}`}>
            <p className="font-bold">Expansión</p>
            <p>{optimization.adequate_expansion === 'yes' ? '✓' : '—'}</p>
          </div>
          <div className={`p-2 rounded-lg text-[9px] font-mono ${optimization.significant_malapposition === 'no' ? 'bg-emerald-950/60 text-emerald-400' : 'bg-card text-muted-foreground'}`}>
