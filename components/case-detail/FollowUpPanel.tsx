...
export default function FollowUpPanel({ caseId, followups = [] }: FollowUpPanelProps) {
  ...
  const getNodeColor = (type: string) => {
    ...
    return 'bg-card/60 border-border text-muted-foreground';
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-bold text-foreground mb-1">Actividad del Caso</h2>
          <p className="text-xs text-muted-foreground">Timeline de outcomes y eventos adversos mayores (MACE)</p>
        </div>
        <Link
          href={`/cases/${caseId}/follow-up`}
          className="px-4 py-2 bg-background hover:bg-muted border border-border rounded-xl text-xs font-semibold text-muted-foreground transition-all"
        >
          Ver Detalles →
        </Link>
      </div>
          ...
          {timepoints.map((type, idx) => {
            ...
            return (
              <div key={type} className="flex flex-col items-center">
                ...
                <p className="mt-3 text-xs font-bold text-muted-foreground text-center">{getLabelForType(type)}</p>
            ...
            return (
              <div key={type} className="flex gap-4 items-start">
                ...
                <div className="flex-1 pt-1">
                  <p className="text-sm font-bold text-muted-foreground">{getLabelForType(type)}</p>
                  {fu && (
      ...
      {followups.length > 0 && (
        <div>
          <h3 className="text-sm font-bold text-foreground mb-3">Resumen de Eventos (Todos los Periodos)</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {/* MACE */}
            <div className="bg-card border border-border rounded-xl p-3 text-center">
              <p className="text-[9px] font-mono text-muted-foreground uppercase mb-2">MACE</p>
            ...
            {/* TLR */}
            <div className="bg-card border border-border rounded-xl p-3 text-center">
              <p className="text-[9px] font-mono text-muted-foreground uppercase mb-2">TLR</p>
            ...
            {/* TVR */}
            <div className="bg-card border border-border rounded-xl p-3 text-center">
              <p className="text-[9px] font-mono text-muted-foreground uppercase mb-2">TVR</p>
            ...
            {/* Rehospitalization */}
            <div className="bg-card border border-border rounded-xl p-3 text-center">
              <p className="text-[9px] font-mono text-muted-foreground uppercase mb-2">Rehosp.</p>
              <p
            ...
            {/* Completed */}
            <div className="bg-card border border-border rounded-xl p-3 text-center">
              <p className="text-[9px] font-mono text-muted-foreground uppercase mb-2">Completados</p>
      ...
      {followups.length === 0 && (
        <div className="bg-background/60 border border-border rounded-2xl p-4 text-center">
          <p className="text-sm text-muted-foreground">No hay seguimientos registrados aún</p>
          <Link
            href={`/cases/${caseId}/follow-up`}
