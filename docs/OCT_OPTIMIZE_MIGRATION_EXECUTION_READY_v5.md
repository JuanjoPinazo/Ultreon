# Ejecución Segura de Migración: OCT-Optimize ULTREON 3.0 (v5)
...
SELECT 
  ...
  h.short_name,
  CASE 
    WHEN h.short_name IS NULL THEN 'ERROR: short_name NULL'
    WHEN h.short_name = '' THEN 'ERROR: short_name vacío'
    WHEN NOT (UPPER(TRIM(h.short_name)) ~ '^[A-Z0-9]+$') THEN 'ERROR: caracteres inválidos'
    WHEN LENGTH(UPPER(TRIM(h.short_name))) > 10 THEN 'ERROR: longitud > 10'
    ELSE 'OK'
...
**Cambios v5:**
- ✅ Preflight 1: Revisar SOLO hospitales activos (IS TRUE)
- ✅ Preflight 1: Devolver SOLO hospitales activos con problemas
- ✅ Rollback: PL/pgSQL válido (DO $rollback$ ... $rollback$ LANGUAGE plpgsql)
- ✅ Rollback: to_regprocedure() para verificar función
- ✅ Rollback: Verificación de tabla antes de DROP POLICY
- ✅ Rollback: DROP INDEX IF EXISTS mantenido
- ✅ Rollback: Columnas nuevas NO eliminadas
- ✅ No hay BEGIN/EXCEPTION fuera de PL/pgSQL
